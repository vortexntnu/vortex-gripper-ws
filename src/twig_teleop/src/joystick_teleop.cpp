...
public:
        ...
        std::unique_ptr<control_msgs::msg::JointJog>& jog)
    {
        ...
        jog->velocities.push_back(
            getDoubleParam(PARAM_SHOULDER_MAX_SPEED) * ignoreDeadband(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_SHOULDER_AXIS).value_or(0)));

        jog->joint_names.push_back("twig_wrist_joint");
        jog->velocities.push_back(
            getDoubleParam(PARAM_WRIST_MAX_SPEED) * ignoreDeadband(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_WRIST_AXIS).value_or(0)));

        jog->joint_names.push_back("twig_left_finger_joint");
        jog->velocities.push_back(
            getDoubleParam(PARAM_GRIPPER_MAX_SPEED) * ignoreDeadband(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_GRIPPER_AXIS).value_or(0)));
    }
