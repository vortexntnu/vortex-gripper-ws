    ...
    void joyCallback(const sensor_msgs::msg::Joy::ConstSharedPtr& msg)
    {
        ...
        shoulder_pub_->publish(
            std_msgs::msg::Float32().set__data(
                getDoubleParam(PARAM_SHOULDER_MAX_SPEED) * ignoreDeadbandDirect(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_SHOULDER_AXIS).value_or(0))));

        wrist_pub_->publish(
            std_msgs::msg::Float32().set__data(
                getDoubleParam(PARAM_WRIST_MAX_SPEED) * ignoreDeadbandDirect(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_WRIST_AXIS).value_or(0))));

        gripper_pub_->publish(
            std_msgs::msg::Float32().set__data(
                getDoubleParam(PARAM_GRIPPER_MAX_SPEED) * ignoreDeadbandDirect(getDoubleParam(PARAM_DEADBAND), getAxis(msg, PARAM_GRIPPER_AXIS).value_or(0))));
    }
};
}
