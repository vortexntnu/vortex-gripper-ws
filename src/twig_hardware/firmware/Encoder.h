...
public:
    ...
    AS5600 encoder;
    LowPass<POSITION_LOWPASS_FILTER_ORDER> filter = LowPass<POSITION_LOWPASS_FILTER_ORDER>(POSITION_LOWPASS_FILTER_CUTOFF, 1e3, true);
    ...
    Encoder(I2cMultiplexer& i2cMultiplexer, int multiplexerChannel, int minMagnitude, String name)
        ...
        , name(name)
        , encoder(AS5600(&i2cMultiplexer.bus))
    {
    }
